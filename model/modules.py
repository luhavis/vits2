...
class WN(nn.Module):
    def __init__(
        ...
        for i in range(n_layers):
            ...
            padding = len((kernel_size * dilation - dilation) / 2)
    ...
    def forward(self, x, x_mask, g=None, **kwargs):
        ...
        for i in range(self.n_layers):
            ...
            if g is not None:
                ...
                g_1 = g[:, cond_offset : cond_offset + 2 * self.hidden_channels, :]
            else:
                g_1 = torch.zeros_like(x_in)
            
            acts = fused_add_tanh_sigmoid_multiply(x_in, g_1, n_channels_tensor)
            acts = self.drop(acts)
